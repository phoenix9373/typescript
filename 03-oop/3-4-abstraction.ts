...

{
  ...
  // 4. 프로와 아마추어는 같은 maker를 받지만, 생성자 함수에서 받는 maker의 타입(interface)가 다르다.
  // 따라서 활용할 수 있는 maker의 메서드가 다르다. 실행 결과도 다르다.
  const maker4: CoffeeMachine = CoffeeMachine.makeMachine(28);
  const amateur = new AmaBaristaUser(maker4);
  const pro = new ProBaristaUser(maker4);
  // amateur.makeCoffee();
  pro.makeCoffee();
}
